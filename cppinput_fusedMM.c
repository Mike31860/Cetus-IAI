...
#define fabs(x) ((x) >= 0 ? (x) : -(y))
   ...
   )
   {
      ...
      int status = 0;
      struct timeval start_fuseMM_csr, end_fuseMM_csr;
      gettimeofday(&start_fuseMM_csr, NULL);
       ...
       */
      // printf( "outside00");
      if (GET_VOP_FLAG(imessage) == VOP_COPY_RHS && GET_ROP_FLAG(imessage) == ROP_NOOP && GET_SOP_FLAG(imessage) == SOP_NOOP && GET_VSC_FLAG(imessage) == VSC_NOOP && GET_AOP_FLAG(imessage) == AOP_ADD)
      {
         printf("xsOptFusedMMtime_gcn_pt \n");
#ifdef DREAL
         printf("dgfusedMM_csr\n");
         dgfusedMM_csr('g', m, n, k, alpha, nnz, rows, cols, val,
                       indx, pntrb, pntre, x, ldx, y, ldy, beta, z, ldz);
#else
         printf("sgfusedMM_csr\n");
         sgfusedMM_csr('g', m, n, k, alpha, nnz, rows, cols, val,
                       indx, pntrb, pntre, x, ldx, y, ldy, beta, z, ldz);
#endif
         gettimeofday(&end_fuseMM_csr, NULL);
         //printf("TimeInstrument_SIMD_PARALLEL_xsOptFusedMMtime_gcn_pt = %0.8f \n", (((&end_fuseMM_csr)->tv_sec - (&start_fuseMM_csr)->tv_sec) + (1.0E-6 * ((&end_fuseMM_csr)->tv_usec - (&start_fuseMM_csr)->tv_usec))));
         return status;
      }
      /*
       * Check for SPMM
       */
      if (GET_VOP_FLAG(imessage) == VOP_COPY_RHS && GET_ROP_FLAG(imessage) == ROP_NOOP && GET_SOP_FLAG(imessage) == SOP_COPY && GET_VSC_FLAG(imessage) == VSC_MUL && GET_AOP_FLAG(imessage) == AOP_ADD)
      {
         printf("xsOptFusedMMtime_spmm_pt\n");
#ifdef DREAL
...
#endif
         gettimeofday(&end_fuseMM_csr, NULL);
         printf("TimeInstrument_Serial_xsOptFusedMMtime_spmm_pt = %0.8f \n", (((&end_fuseMM_csr)->tv_sec - (&start_fuseMM_csr)->tv_sec) + (1.0E-6 * ((&end_fuseMM_csr)->tv_usec - (&start_fuseMM_csr)->tv_usec))));
         return status;
      }
       ...
       */
      if (GET_VOP_FLAG(imessage) == VOP_COPY_RHS && GET_ROP_FLAG(imessage) == ROP_DOT && GET_SOP_FLAG(imessage) == SOP_UDEF && GET_VSC_FLAG(imessage) == VSC_MUL && GET_AOP_FLAG(imessage) == AOP_ADD)
      {
         printf("xsOptFusedMMtime_sigmoid_pt\n");
#ifdef DREAL
...
#endif
         gettimeofday(&end_fuseMM_csr, NULL);
         printf("TimeInstrument_Serial_xsOptFusedMMtime_sigmoid_pt = %0.8f \n", (((&end_fuseMM_csr)->tv_sec - (&start_fuseMM_csr)->tv_sec) + (1.0E-6 * ((&end_fuseMM_csr)->tv_usec - (&start_fuseMM_csr)->tv_usec))));
         return status;
      }
       ...
       */
      if (GET_VOP_FLAG(imessage) == VOP_SUBR && GET_ROP_FLAG(imessage) == ROP_NORMR && GET_SOP_FLAG(imessage) == SOP_UDEF && GET_VSC_FLAG(imessage) == VSC_MUL && GET_AOP_FLAG(imessage) == AOP_ADD)
      {
         printf("xsOptFusedMMtime_fr_pt\n");
...
#endif
         gettimeofday(&end_fuseMM_csr, NULL);
         printf("TimeInstrument_Serial_xsOptFusedMMtime_fr_pt = %0.8f \n", (((&end_fuseMM_csr)->tv_sec - (&start_fuseMM_csr)->tv_sec) + (1.0E-6 * ((&end_fuseMM_csr)->tv_usec - (&start_fuseMM_csr)->tv_usec))));
         return status;
      }
