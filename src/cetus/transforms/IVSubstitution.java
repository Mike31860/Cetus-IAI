...
public class IVSubstitution extends TransformPass {
    ...
    */
    ...
    /**
    * Options for IV substitution.
    */
    private static final int IND_LINEAR = 1;
    private static final int IND_NONLINEAR = 2;
    private static final int IND_ALLOW_NZT_TEST = 3;
    private int option;
    ...
    */
    public IVSubstitution(Program program) {
        super(program);
        // Assumption: caller of this constructor already checked IV
        // substitution is turned on.
        try {
            option = Integer.parseInt(Driver.getOptionValue("induction"));
        } catch (NumberFormatException ex) {
            option = IND_ALLOW_NZT_TEST;
        } finally {
            if (option < IND_LINEAR || option > IND_ALLOW_NZT_TEST) {
                option = IND_ALLOW_NZT_TEST;
            }
        }
    }
    ...
    */
    public void start() {
        ...
        while (iter.hasNext()) {
            ...
            if (t instanceof TranslationUnit) {
                ...
                if (result == WRAP) {
                    ...
                    result = analyze(loop);
                    //System.out.println(getReport(loop));
        ...
        System.out.println("IV ID Class : "+loop_to_ivs.size());
    ...
    private Expression getNZTConditions(Loop loop) {
        ...
        for (int i = 0; i <= eligible_loops.indexOf(loop); i++) {
            ...
            if (IRTools.isAncestorOf(loop, curr_loop) &&
                !loop_to_stmts.get(curr_loop).isEmpty()) {
    ...
    */
    private int transform(Loop loop) {
        ...
        if (nzt_condition != null) {
            // Do not insert runtime test as requested.
            if (option < IND_ALLOW_NZT_TEST) {
                return PASS;
            }
            ...
            for (Symbol iv : ivs) {
                ...
                if (inc == null) {
                    ...
                    PrintTools.printlnStatus(2, tag,
                            "Skipping replacement of", iv.getSymbolName(),
                            "due to complicated increments");
                    continue;
                }
                // Discards IVs that produces non-linear expressions as
                // requested by the user. The criterion is:
                //     if there exists any polynomial of loop index whose
                //     degree is larger than 1, among the computed increments.
                if (option < IND_NONLINEAR) {
                    boolean is_nonlinear = false;
                    List<Expression> coef;
                    Expression increment;
                    for (Loop l : loop_to_bounds.keySet()) {
                        Identifier id =
                                (Identifier)loop_to_bounds.get(l).get(0);
                        for (Statement s : inc_after_stmt.keySet()) {
                            increment = inc_after_stmt.get(s);
                            coef = Symbolic.getPolynomialCoef(increment, id);
                            if (coef == null || coef.size() > 2) {
                                is_nonlinear = true;
                                break;
                            }
                        }
                        for (Loop ll : inc_into_loop.keySet()) {
                            increment = inc_into_loop.get(ll);
                            coef = Symbolic.getPolynomialCoef(increment, id);
                            if (coef == null || coef.size() > 2) {
                                is_nonlinear = true;
                                break;
                ...
                if (replace(curr_loop, iv, init) == PASS) {
                    PrintTools.printlnStatus(2, tag, "Successfully replaced",
                            iv.getSymbolName());
                }
    ...
    */
	...
	public void setAll_loops(ArrayList<Loop> all_loops) {
