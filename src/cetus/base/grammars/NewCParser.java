...
String index ;
Declaration loop_decl = null;
Declarator decl_for_global = null;
ArrayList<IDExpression> loop_idex_list = new ArrayList<IDExpression>();
ArrayList<Declaration> loop_decl_list = new ArrayList<Declaration>();
Boolean extern_decl = false;
 ...
 ****************************************************/
	public final Specifier  typeSpecifier() throws RecognitionException, TokenStreamException {
		Specifier types;
		Specifier struct_specifier = null;
...
inputState.guessing--;
		...
		try {      // for error handling
			...
			do {
				if ((LA(1)==StringLiteral)) {
					...
					if ( _cnt339>=1 ) { break _loop339; } else {throw new NoViableAltException(LT(1), getFilename());}
				}
	...
	public final Declarator  initDecl() throws RecognitionException, TokenStreamException {
		Declarator decl;
		IDExpression lhsID = null;
		Declarator init_decl = null;
		decl = null;
	...
	public final Expression ForinitDecl() throws RecognitionException, TokenStreamException{

		Expression init_for_expr = null;
		
		IDExpression idex_for = null;
		List  dlist_for = null;
		Declaration decl_for = null;
		List dspec_for_list = null;
		Specifier dspec = null;


		try{
			
			dspec_for_list = declSpecifiers();
			switch(LA(1)){
					...
					case CharLiteral:
					{
							...
							init_for_expr =expr();
							idex_for = new NameID(index);

				
							if(hastypedef) {
								typetable.put(index,"typedef");
							}
							break;
					}
...
inputState.guessing--;
				...
				match(COLON);
				{
				if ((_tokenSet_66.member(LA(1))) && (_tokenSet_74.member(LA(2)))) {
					stmt1=statement();
					if ( inputState.guessing==0 ) {
			...
			default:
				if ((_tokenSet_17.member(LA(1))) && (_tokenSet_69.member(LA(2)))) {
					stmtb_expr=expr();
					exprsemi = LT(1);
					match(SEMI);
					if ( inputState.guessing==0 ) {
...
inputState.guessing--;
			...
			else if ((_tokenSet_17.member(LA(1))) && (_tokenSet_89.member(LA(2)))) {
				ret_expr=unaryExpr();
			}
			else {
				throw new NoViableAltException(LT(1), getFilename());
		...
		try {      // for error handling
			...
			do {
				if ((LA(1)==COMMA)) {
					...
					else if ((_tokenSet_2.member(LA(1))) && (_tokenSet_90.member(LA(2)))) {
						pdecl=parameterDeclaration();
						if ( inputState.guessing==0 ) {
							eList.add(pdecl);
						}
	...
	public final Expression  gnuAsmExpr() throws RecognitionException, TokenStreamException {
		...
		try {      // for error handling
			...
			if ((LA(1)==COLON) && (_tokenSet_91.member(LA(2)))) {
				...
				else if ((LA(1)==COLON||LA(1)==RPAREN) && (_tokenSet_92.member(LA(2)))) {
				}
				else {
					throw new NoViableAltException(LT(1), getFilename());
				}
