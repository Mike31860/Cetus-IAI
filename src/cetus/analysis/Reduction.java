...
/**
 * Performs reduction variable analysis to detect and annotate statements like x
 * = x + i in loops. An Annotation is added right before loops that contain
 * reduction variables.
 */
public class Reduction extends AnalysisPass {
	...
	// Miguel added
	private List<ReductionDTO> listStatements = new ArrayList<ReductionDTO>();
...
*/
        ...
        while (iter.hasNext()) {
			...
			String loopStored =loop.toString();
            Map<String, Set<Expression>> reduce_map = analyzeStatement(loop);         
          ReductionLoops.put(loop, reduce_map);
 ...
 //          System.out.println("\n\n\n-----------------------------------------");
            ...
            List<ReductionDTO> listStatementsCache= new  ArrayList<ReductionDTO>();
            listStatementsCache.addAll(listStatements);          
            LoopReductionStatements.put(loop, listStatementsCache);
	...
	public Map<String, Set<Expression>> analyzeStatement(Statement istmt) {
		...
		int expr_cnt = 0;
		DFIterator<Expression> iter = new DFIterator<Expression>(istmt, Expression.class);
		...
		while (iter.hasNext()) {
			...
			PrintTools.printlnStatus(9, pass_name, "[expr]", ++expr_cnt, ":", expr, "(", expr.getClass().getName(),
					")");
		...
		// Following code block is for detecting possible MIN or MAX reductions in
		// if-statements

		ArrayList<Expression> LoopExprList = new ArrayList<Expression>();
		DFIterator<Expression> loopexpriter = new DFIterator<Expression>(istmt, Expression.class);
		...
		// Following code block is to find min or max reduction using the conditional
		// operator.
		...
		for (String op : rmap.keySet()) {
			for (Expression candidate : rmap.get(op)) {
				...
				if (!RefMap.get(candidate_symbol).isEmpty() && !op.equals("max") && !op.equals("min")) {
					PrintTools.printlnStatus(2, pass_name, candidate, "is referenced in the non-reduction statement!");
					remove_flag = true;
				}
				...
				if (alias != null) {
					DFIterator<Statement> stmt_iter = new DFIterator<Statement>(istmt, Statement.class);
					while (stmt_iter.hasNext()) {
						Statement o = stmt_iter.next();
						if (o instanceof ExpressionStatement || o instanceof DeclarationStatement
								|| o instanceof ReturnStatement) {
							if (alias.isAliased(o, candidate_symbol, RefMap.keySet())) {
								PrintTools.printlnStatus(2, pass_name, candidate, "is Aliased!");
				...
				if (side_effect_set.contains(candidate_symbol)) {
					PrintTools.printlnStatus(2, pass_name, candidate, "has side-effect!");
				...
				if (candidate instanceof ArrayAccess && istmt instanceof ForLoop) {
					 ...
					 * PrintTools.println("loop: " + loop, 2); reduction_set.remove(candidate); }
					 */
					if (simple_self_dependency_check((ArrayAccess) candidate, (ForLoop) istmt)) {
						PrintTools.printlnStatus(2, pass_name, "No self-carried-output dependence in", candidate);
						remove_flag = true;
	 ...
	 * dependency checking part; analyzeStatement() works on outer-most loops, but
	 * this method works on inner-loops (loops contained in a critical section of a
	 * parallel region can be considered as inner-loops with respect to the
	...
	private void add_to_rmap(Map<String, Set<Expression>> rmap, String reduce_op, Expression reduce_expr, Expression rhs) {
		...
		//miguel
	//	System.out.println("Reduction sentence: Op: "+reduce_op +" , lhs: "+reduce_set+", rhs: "+rhs);
		rmap.put(reduce_op, reduce_set);
	}

	private void findReduction(UnaryExpression expr, Map<String, Set<Expression>> rmap,
		...
		if (lhse instanceof IDExpression || lhse instanceof ArrayAccess || lhse instanceof AccessExpression) {
		...
		if (isReduction) {
			addReductionVariablesLoop(lhse, null, reduction_op);
			add_to_rmap(rmap, reduction_op, lhse, null);
			add_to_cmap(cmap, SymbolTools.getSymbolOf(lhse), System.identityHashCode(lhse));
			PrintTools.printlnStatus(2, pass_name, "candidate = (", reduction_op, ":", lhse, ")");
		}
	}

	private void findReduction(AssignmentExpression expr, Map<String, Set<Expression>> rmap,
			Map<Symbol, Set<Integer>> cmap) {
		...
		if (lhse instanceof IDExpression || lhse instanceof ArrayAccess || lhse instanceof AccessExpression) {
			if (assign_op == AssignmentOperator.NORMAL) {
				...
				if (parent_expr instanceof BinaryExpression) {
					...
					if (reduction_op.equals("+")) {
						...
						if (reduction_op.equals("&&") || reduction_op.equals("||") || reduction_op.equals("&")
			...
			if (lhse instanceof Identifier) {
				...
				if (base_array_name instanceof Identifier && array.getIndices().size() == 1) {
		...
		if (isReduction) {
			...
			// System.out.println("Reduction expre: "+expr_set.toString());
			addReductionVariablesLoop(lhse, rhse, reduction_op);

			add_to_rmap(rmap, reduction_op, lhse, rhse);

			for (Expression e : IRTools.findExpressions(expr, lhse)) {
				add_to_cmap(cmap, SymbolTools.getSymbolOf(lhse), System.identityHashCode(e));
			}
			PrintTools.printlnStatus(2, pass_name, "candidate = (", reduction_op, ":", lhse, ")");
		}
	...
	// Miguel added
	public void addReductionVariablesLoop(Expression LHS, Expression rhs, String operator) {

		// Find reduction variable sin a loop
		// Find types of operations in the reduction variable
		// Array Accesses..

		ReductionDTO reductionDTO = new ReductionDTO();
		reductionDTO.setLHS(LHS);
		reductionDTO.setRHS(rhs);
		reductionDTO.setReductionOperator(operator);
		listStatements.add(reductionDTO);

	}

	/*
	 * Method to find Min or MAX reductions. Uses a different way to check if the
	 * min or max candidate variable are used in Non-reduction expressions.
	 */

	private void findReduction(BinaryExpression condexpr, ArrayList<Expression> LoopExpressionList,
			ArrayList<BinaryExpression> Ifexprlist, Map<String, Set<Expression>> rmap, Map<Symbol, Set<Integer>> cmap) {
		...
		// Loop to check the if-condition. If the if-condition contains the loop index
		...
		// Following is the algorithm to check if the reduction candidate is present in
		// any non-reduction statement
		...
		if (isreduction) {
			...
			if (condexpr.getLHS().equals(reduction_candidate) && condexpr.getOperator().toString().equals("<"))
				reduction_operator = "max";

			else if (condexpr.getRHS().equals(reduction_candidate) && condexpr.getOperator().toString().equals(">"))
				reduction_operator = "max";

			else if (condexpr.getLHS().equals(reduction_candidate) && condexpr.getOperator().toString().equals(">"))
		...
		if (isreduction) {
			add_to_rmap(rmap, reduction_operator, reduction_candidate, condrhs);

			for (Expression e : IRTools.findExpressions(reduction_expr, reduction_candidate)) {
				add_to_cmap(cmap, SymbolTools.getSymbolOf(reduction_candidate), System.identityHashCode(e));
			}
			PrintTools.printlnStatus(2, pass_name, "candidate = (", reduction_operator, ":", reduction_candidate, ")");
		}

	}

	/*
	 * Method to find Min and Max reductions in loops with Conditional Expressions.
	 * Similar as the previous method.
	 * 
	 * Slightly different method to check if the reduction candidate is present in a
	 * non-reduction statement.
	 */

	private void findReduction(ConditionalExpression expr, ArrayList<Expression> loopexprlist,
			Map<String, Set<Expression>> rmap, Map<Symbol, Set<Integer>> cmap) {
		...
		if (true_expr instanceof Expression) {
			...
			if (true_expr.equals(rhs_cond_expr) && false_expr.equals(lhs_cond_expr)) {
			...
			else if (true_expr.equals(lhs_cond_expr) && false_expr.equals(rhs_cond_expr)) {
		...
		// Following is used to check if the reduction candidate is present in any
		// non-reduction statement
		...
		for (i = 0; i < loopexprlist.size(); i++) {
			...
			if (loopexprlist.get(i) instanceof AssignmentExpression) {
				...
				if (temp.getRHS() instanceof ConditionalExpression && temp.getRHS().equals(expr)) {
...
// Restriction on reduction: True expression has to be the reduction candidate
		...
		if (isreduction) {
			...
			if (true_expr.equals(reduction_candidate) && lhs_cond_expr.equals(reduction_candidate)
					&& Binary_cond_expr.getOperator().toString().equals(">"))
				reduction_operator = "max";

			else if (true_expr.equals(reduction_candidate) && rhs_cond_expr.equals(reduction_candidate)
					&& Binary_cond_expr.getOperator().toString().equals("<"))
				reduction_operator = "max";

			else if (true_expr.equals(reduction_candidate) && lhs_cond_expr.equals(reduction_candidate)
					&& Binary_cond_expr.getOperator().toString().equals("<"))
				reduction_operator = "min";
		...
		if (isreduction && reduction_operator != null) {
			add_to_rmap(rmap, reduction_operator, reduction_candidate, rhs_cond_expr);
	...
	public Map<Loop, List<ReductionDTO>> getLoopReductionStatements() {
		return LoopReductionStatements;
	}

	public void setLoopReductionStatements(Map<Loop, List<ReductionDTO>> loopReductionStatements) {
		LoopReductionStatements = loopReductionStatements;
	}

	public Map<Loop, Map<String, Set<Expression>>> getReductionLoops() {
		return ReductionLoops;
	}

	public void setReductionLoops(Map<Loop, Map<String, Set<Expression>>> reductionLoops) {
