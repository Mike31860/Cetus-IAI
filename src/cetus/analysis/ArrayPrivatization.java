 ...
 * then read in a loop body. The high-level algorithm in below describes the
 * process of detecting privatizable variables, both scalars and array sections,
 * in a loop. The set operations that appear in the algorithm are performed on
 * the array sections if the variable is an array. We use the power of symbolic
 * analysis techniques in Cetus to make the symbolic section operation possible.
 * For example, <code>[1:m] (intersect) [1:n]</code> results in
 * <code>[1:n]</code> if the expression comparison tool with the value range set
 * can decide <code>n</code> is less than or equal to <code>m</code>.
 * <p>
 ...
 * <code>upward exposed</code>, creating read references seen in the outer loop.
 * Second, the algorithm aggregates all these array sections over the loop
 * iteration space, creating the array sections that are private, written and
 * upward exposed for the entire loop. The aggregation for the written sections
 * (<code>DEF</code>) computes the must-defined regular sections of the arrays
 * over the entire loop iteration while the aggregation of upward-exposed
 * sections (<code>UEU</code>) requires only the conservative value ranges of
 * the sections (may-used sections). This algorithm is a slightly simpler
 * version of the one used in the Polaris parallelizing compiler for Fortran77
 * programs.
 * 
 * <pre>
 ...
 * <code>-privatize</code>, and the result is stored as an annotation that
 * contains the set of private variables. We do not consider a variable with
 * user-defined type as a candidate private variable, but intend to widen the
 * scope of the analysis in the future.
 */
public class ArrayPrivatization extends AnalysisPass {
	...
	// Miguel
	private ArrayList<Expression> expressionsLoop;

	// method that storage all loops
	...
	// Suboptions
	private static final int PRIV_SCALAR = 1;
	private static final int PRIV_ARRAY = 2;
	private int option;

	private LoopInfo loopInfo;
	private ProfitableOMP profitableOMP;

	private Map<Loop, AnalysisLoopTarget> loopsPrivatization;
	...
	public ArrayPrivatization(Program program) {
		super(program);
		loopsPrivatization = new HashMap<Loop, AnalysisLoopTarget>();
		assignmentExpressionsMaps = new HashMap<Expression, Expression>();
		expressionsLoop = new ArrayList<Expression>();
		debug = PrintTools.getVerbosity();
		try {
			option = Integer.parseInt(Driver.getOptionValue("privatize"));
		} catch (NumberFormatException ex) {
			option = PRIV_ARRAY;
		} finally {
			if (option < PRIV_SCALAR || option > PRIV_ARRAY) {
				option = PRIV_ARRAY;
			}
		}
		// Call alias analysis.
		...
		profitableOMP = new ProfitableOMP(program, 0);
		AllLoops = new ArrayList<Loop>();
	}
	...
	public void start() {
		...
		loopCondition();
		DFIterator<Procedure> iter = new DFIterator<Procedure>(program, Procedure.class);
		iter.pruneOn(Procedure.class);

		while (iter.hasNext()) {
			Procedure p = iter.next();
			if (alias_result != 0) {
				PrintTools.printlnStatus(0, tag, "[WARNING] Privatization stops due to all-to-all alias");
				break;
			}
			analyzeProcedure(p);
		}
	...
	// Miguel
	public void loopCondition() {
		DFIterator<Procedure> iter = new DFIterator<Procedure>(program, Procedure.class);
		iter.pruneOn(Procedure.class);
		while (iter.hasNext()) {
			Procedure p = iter.next();
			DFIterator<Loop> iter2 = new DFIterator<Loop>(p, Loop.class);
			iter2.pruneOn(Loop.class);
			iter2.pruneOn(StatementExpression.class); // We do not support this.
			while (iter2.hasNext()) {
				Loop loop = iter2.next();
				addConditionsLoopExpression(loop);
	...
	/**
	 * Starts array privatization for a procedure. It first computes the range map
	 * of the procedure to enable subsequent symbolic operations, then calls
	 * {@link #analyzeLoop(Loop)} to analyze the outer-most loops in the procedure.
	 * The final step is to annotate the analysis result for each loop in the
	 * procedure.
	 *
	 * @param proc the input procedure.
	 */
	public void analyzeProcedure(Procedure proc) {
		// System.out.println("----------------------/n"+proc.toString());
		...
		addAssigmentExpressions(proc);
		addAnnotation(proc);
		PrintTools.printlnStatus(1, tag, String.format("...... %.2f seconds", Tools.getTime(timer)));
	}

	// Miguel
	public void addConditionsLoopExpression(Loop loop) {
		Expression expre = loop.getCondition();
		LoopTools.storeExpressionLoopIndex((Expression) expre.getChildren().get(1),
				expressionsLoop);
		...
		while (iter.hasNext()) {
			...
			addConditionsLoopExpression(neeLoop);
		}

	}

	public void addAssigmentExpressions(Procedure proc) {
		DFIterator<Expression> Statement = new DFIterator<Expression>(proc, Expression.class);
		Statement.pruneOn(AssignmentExpression.class);
		if(proc.toString().startsWith("int main")){
			System.out.println("hay");
		}
		while (Statement.hasNext()) {
			Expression assigment = Statement.next();
			// System.out.println(assigment.toString());
			if (assigment.getChildren().size() > 1) {
				Expression left = (Expression) assigment.getChildren().get(0);
				Expression right = (Expression) assigment.getChildren().get(1);
				if (expressionsLoop.contains(left) && !assignmentExpressionsMaps.containsKey(left)
						&& !left.toString().matches(".*[0-9].*")) {
					assignmentExpressionsMaps.put(left, right);
				}
	...
	/**
	 * Creates a cetus private annotation for each loop and inserts it before the
	 * loop.
	 */
	public void addAnnotation(Procedure proc) {
		...
		for (Loop loop : pri_map.keySet()) {
			...
			for (Symbol var : keys) {
				Set alias_set = alias_analysis.get_alias_set((Statement) loop, var);
				if (alias_set == null) {
					continue;
				}
				for (Object aliased : alias_set) {
					if (aliased.equals("*")) {
						...
						PrintTools.printlnStatus(1, tag, "Removing aliased variable:", var, "=>", aliased);
		...
		for (Loop loop : pri_map.keySet()) {
			...
			postPrivatize(live_cfg, loop_stmt, loop_pri_set, loop_live_set);
		}
	}

	/**
	 * Performs post-processing of array privatization by checking safe conditions
	 * and discarding array variables if they are unsafe or array privatization is
	 * not requested. Post-processing finalizes privatization by annotating the
	 * given loop with appropriate cetus annotation.
	 * 
	 * @param cfg      the CFG used for safety checking.
	 * @param loop     the loop to be processed.
	 * @param pri_set  the analyzed private variables.
	 * @param last_set the analyzed live private variables.
	 */
	private void postPrivatize(CFGraph cfg, Statement loop, Set<Symbol> pri_set, Set<Symbol> last_set) {
		if (!(loop instanceof ForLoop)) {
			return;
		}
		Set<Symbol> aggregates = new HashSet<Symbol>(pri_set);
		aggregates.addAll(last_set);
		...
		while (iter.hasNext()) {
			...
			List specs = SymbolTools.getNativeSpecifiers(loop, var);
			if (!Tools.containsClasses(specs, PointerSpecifier.class, ArraySpecifier.class)) {
				iter.remove();
			}
		}
		if (option == PRIV_SCALAR) { // user-specified option
			pri_set.removeAll(aggregates);
			last_set.removeAll(aggregates);
		} else {
			// needs further analysis for safe array privatization:
			// 1) pointer parameters should be avoided.
			// 2) firstprivate/lastprivate for global arrays and arrays having
			// reaching definitions before the loop.
			// collects defined variables including initializers.
			Set<Symbol> defs = new HashSet<Symbol>();
			List<DFANode> visited = new ArrayList<DFANode>();
			int prev_size = 0;
			DFANode loop_node = cfg.getNodeWith("stmt", loop);
			if (loop_node != null) {
				visited.add(loop_node);
			}
			while (prev_size < visited.size()) {
				prev_size = visited.size();
				DFANode curr = visited.get(visited.size() - 1);
				for (DFANode pred : curr.getPreds()) {
					if (visited.contains(pred)) {
						continue;
					}
					Object ir = CFGraph.getIR(pred);
					if (ir instanceof Traversable) {
						defs.addAll(DataFlowTools.getDefSymbol((Traversable) ir));
					}
					visited.add(pred);
				}
			}
			// initializers
			SymbolTable st = IRTools.getAncestorOfType(loop, SymbolTable.class);
			while (st != null && !(st instanceof Procedure)) {
				for (Symbol symbol : SymbolTools.getSymbols(st)) {
					if (symbol instanceof Declarator && ((Declarator) symbol).getInitializer() != null) {
						defs.add(symbol);
					}
				}
				st = IRTools.getAncestorOfType(st, SymbolTable.class);
			}
			// collects firstprivate variables
			iter = last_set.iterator();
			while (iter.hasNext()) {
				Symbol symbol = iter.next();
				if (aggregates.contains(symbol)) {
					if (defs.contains(symbol) || SymbolTools.isGlobal(symbol) || SymbolTools.isFormal(symbol)) {
						first_set.add(symbol);
					}
				}
			}
			iter = pri_set.iterator();
			while (iter.hasNext()) {
				Symbol symbol = iter.next();
				if (aggregates.contains(symbol)) {
					if (defs.contains(symbol) || SymbolTools.isGlobal(symbol) || SymbolTools.isFormal(symbol)) {
		...
		if (!first_set.isEmpty()) {
			...
			note.put("firstprivate", first_set);
			loop.annotateBefore(note);
		}
		if (!pri_set.isEmpty()) {
			note = new CetusAnnotation();
			note.put("private", pri_set);
			loop.annotateBefore(note);
		}
		if (!last_set.isEmpty()) {
			note = new CetusAnnotation();
			note.put("lastprivate", last_set);
			loop.annotateBefore(note);
		}
	}
	...
	private void analyzeLoop(Loop loop) {
		...
		PrintTools.printlnStatus(2, PrintTools.line_sep, tag, "Loop:", LoopTools.toControlString(loop));
		// ------------------------------
		...
		aggregateDef();
		aggregateUse();
		// Miguel you need to use the loop entity because this is the one that will be
		// updated
		AllLoops.add(loop);
		AnalysisLoopTarget loopTarget = new AnalysisLoopTarget();/*  */
		for (Symbol a : loop_variants) {
			if (((Object) a) instanceof double[]) {
				loopTarget.getArrayVariables().add(a);
			}
		}

		loopTarget.setLoop(loop);
		loopTarget.setFuncitonCalls(LoopTools.numberFunctionCall(loop));
		// loopTarget.setSizeLoop(LoopTools.calculateLoopSize(loop));
		loopTarget.setLoopNestLevel(LoopTools.calculateLoopNest(loop).size());
		loopTarget.setNumberPV(pri_set.size());
		loopTarget.setPri_set(pri_set);
		loopTarget.setVariables(loop_variants);
		loopTarget.setNumberStatements(numberStatements);
		loopsPrivatization.put(loop, loopTarget);
	}
	 ...
	 * the loop body analysis. It could be easier to create the CFG of the compound
	 * statement (loop body) but we also need to visit the nodes that control the
	 * loop for more general analysis of the loop.
	 */
	private CFGraph buildLoopGraph() {
		...
		if (current_loop instanceof DoLoop) {
			...
			Set<DFANode> back_from_nodes = new LinkedHashSet<DFANode>(condition_node.getPreds());
	...
	private void addUseFromDeclaration() {
		...
		DFIterator<Initializer> iter = new DFIterator<Initializer>(current_loop.getBody(), Initializer.class);
		iter.pruneOn(Loop.class);
		while (iter.hasNext()) {
		...
		while (!work_list.isEmpty()) {
			...
			for (DFANode pred : node.getPreds()) {
				...
				if (curr_map == null) {
					...
					curr_map = curr_map.intersectWith(pred_map, (RangeDomain) node.getData("range"));
				}
			}
	 ...
	 * variables; if a defined variable is a scalar or an array with loop-invariant
	 * subscript expression, it is a candidate. If the node contains any function
	 * calls, the following conservative decision is made: Every global variables
	 * and actual parameters of which addresses are taken are considered as being
	 * modified.
	 *
	 * This decision affects the analysis in two aspects: 1) Array section
	 * containing the above variables should be killed. 2) Used array sections
	 * containing the above variables are overly approximated and we handle this
	 * case by removing the DEF array section entries for the variables
	 */
	 ...
	 *
	 * IN[n] = (V) { OUT[p], p in predecessors of n } OUT[n] = IN[n] (^) GEN[n] (v):
	 * join like constant propagation (^): add induction increments
	 *
	 * Each variables' states are initialized to 0. Non-induction assignment just
	 * kills the data. Example with OUT data: i=i+1 : i -&gt; 1 i=i+2 : i -&gt; 3
	 * i=k : i -&gt; TOP
	 */
	...
	/**
	 * Returns the result of a join operation of the two induction map. e1 v e2 = e1
	 * if e1==e2, = T otherwise
	 */
	private Map<Symbol, Expression> joinInductionMap(Map<Symbol, Expression> m1, Map<Symbol, Expression> m2) {
	...
	/**
	 * Performs a meet operation in the specified node (transfer function). e1 ^ e2
	 * = e1+e2 if e1!=T and e2!=T = T otherwise
	 */
		...
		while (iter.hasNext()) {
			...
			if (e instanceof AssignmentExpression) {
				...
				if (ae.getLHS() instanceof Identifier
						&& SymbolTools.isInteger(((Identifier) ae.getLHS()).getSymbol())) {
					...
					if (ao == AssignmentOperator.NORMAL) {
						...
						diff = Symbolic.subtract(new IntegerLiteral(0), ae.getRHS());
					} else {
						diff = top;
					}
				}
				...
				if (ue.getExpression() instanceof Identifier
						&& SymbolTools.isInteger(((Identifier) ue.getExpression()).getSymbol())) {
	 ...
	 * function's callee. Scalars are not considered in this method because whether
	 * they (scalars) are used is not important for the analysis; remember that this
	 * is for conservative array section operations in the absence of any
	...
	private Set<Symbol> getMayUseFromFC(Traversable tr, Set<Symbol> vars) {
		...
		DFIterator<FunctionCall> iter = new DFIterator<FunctionCall>(tr, FunctionCall.class);
	...
	/**
	 * Collects candidate private variables from the section map by checking if they
	 * do not contain any loop-variant variables.
	 */
	...
	/**
	 * Collects privatizable variables. The candidate private variables have already
	 * been stored in the pri_map for the analyzer. If the summary UEU set of each
	 * loop contains any candidate variable, that variable is removed from the
	 * candidate list. Another set of candidates come from the may_set which
	 * contains the defined set of the inner loops with the execution of the loop
	 * not guaranteed at compile time. This set needs to be added to the private
	 * list after excluding UEU set.
	 */
	private void collectPrivateSet(CFGraph g) {
		pri_set = pri_map.get(current_loop);
		...
		if (current_loop instanceof ForLoop) {
			...
			while (node_iter.hasNext()) {
				...
				// 3. ReturnStatement.
				if (ir instanceof GotoStatement && g.getNode("ir", ((GotoStatement) ir).getTarget()) == null
						|| ir instanceof BreakStatement && node.getSuccs().isEmpty() || ir instanceof ReturnStatement) {
					pri_set.clear();
					continue;
				}
		...
		DFIterator<Loop> iter = new DFIterator<Loop>(current_loop.getBody(), Loop.class);
		iter.pruneOn(Loop.class);
		iter.pruneOn(StatementExpression.class);
		while (iter.hasNext()) {
			Set<Symbol> inner_pri = new LinkedHashSet<Symbol>(pri_map.get(iter.next()));
	...
	/**
	 * Removes variables from the set if they appear in function call or they are
	 * global when a function call exists.
	 */
	private void removeMayUsedVariables(Set<Symbol> vars) {
		boolean contains_unsafe_fcall = false;
		DFIterator<FunctionCall> iter = new DFIterator<FunctionCall>(current_loop, FunctionCall.class);
		while (iter.hasNext()) {
			FunctionCall fc = iter.next();
			...
			// Referencing
			List<UnaryExpression> address_of = IRTools.getUnaryExpression(fc, UnaryOperator.ADDRESS_OF);
	...
	/**
	 * Computes set of live variables at each program point with the given mask_set
	 ...
	 */
	private void computeLiveVariables(DFANode node, Set<Symbol> gen, Set<Symbol> kill, Set<Symbol> mask_set) {
	...
	private Section.MAP getSectionMap(Expression e, boolean def) {
		...
		if (e instanceof ArrayAccess) {
			...
			if (var instanceof AccessSymbol) {
				...
				ret.put(SymbolTools.getSymbolOf(aa.getArrayName()), new Section(aa));
			}
	...
	/**
	 * For now, just union them once in reverse post order. This method is
	 * inefficient because we need to rebuild RangeDomain for intermediate nodes
	 * which do not represent a statement in the IR. It seems like this is the only
	 * way to provide range information correctly.
	 */
	private void getRangeDomain(CFGraph g) {
		...
		for (Integer order : work_list.keySet()) {
			...
			if (rd != null) {
				...
				RangeDomain range = RangeAnalysis.query((Statement) current_loop);
				if (range == null) {
	...
	public Map<Loop, AnalysisLoopTarget> getLoopsPrivatization() {
		return loopsPrivatization;
	}

	public void setLoopsPrivatization(Map<Loop, AnalysisLoopTarget> loopsPrivatization) {
		this.loopsPrivatization = loopsPrivatization;
	}
	...
	public Map<Expression, Expression> getAssignmentExpressionsMaps() {
		return assignmentExpressionsMaps;
	}
