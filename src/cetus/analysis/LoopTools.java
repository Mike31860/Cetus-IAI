...
public class LoopTools {
    ...
    // Miguel
    public static Expression getUpperExpression(Loop loop) {
        Expression ub = null;
        if (loop instanceof ForLoop) {
            ForLoop for_loop = (ForLoop) loop;
            // determine upper bound for index variable of this loop
    ...
    // Miguel
    public static void getnumberOperationsLoop(Loop loop) {
        /*
...
//Miguel
    ...
    public static int calculateLoopSize(Loop loop) {
        int stamt = 0;
...
//Miguel
    ...
    private CFGraph buildLoopGraph(Loop current_loop) {
        ...
        getRangeDomain(ret, current_loop);
        // if (debug >= 3) {
    ...
    private void getRangeDomain(CFGraph g, Loop current_loop) {
    ...
    public static boolean scalarDependencePossible(Loop l) {
        ...
        // This functionality must be handled by DataFlowTools getDefSymbols()
        // in the future
        DFIterator<AssignmentExpression> dfs_iter = new DFIterator<AssignmentExpression>(
        ...
        while (dfs_iter.hasNext()) {
            ...
            if (lhs instanceof UnaryExpression &&
                    ((UnaryExpression) lhs).getOperator() == UnaryOperator.DEREFERENCE) {
    ...
    // Miguel
    public static void storeExpressionLoopIndex(Expression expre, ArrayList<Expression> expressionsLoop) {
        if (expre.getChildren().size() > 0) {
            for (int index = 0; index < expre.getChildren().size(); index++) {
                Expression exp = (Expression) expre.getChildren().get(index);
                storeExpressionLoopIndex(exp, expressionsLoop);
            }
        }

        if (!isNumeric(expre.toString()) && expre.getChildren().size() == 0 && !expressionsLoop.contains(expre)) {
            expressionsLoop.add(expre);
        }
    }
    ...
    public static LinkedList<Loop> calculateInnerLoopNest(Loop enclosing_loop) {
        ...
        DepthFirstIterator<Traversable> dfs_iter = new DepthFirstIterator<Traversable>((Traversable) enclosing_loop);
        for (;;) {
            Loop l = null;
            try {
    ...
    private static void addLoopName(
        ...
        Map<Statement, PragmaAnnotation> names = new HashMap<Statement, PragmaAnnotation>();
        ...
        for (int i = 0; i < children_size; i++) {
            ...
            if (t == null) {
                ...
                LinkedList<Integer> init_nums = new LinkedList<Integer>(Arrays.asList(new Integer(0)));
    ...
    public static boolean isPrivate(Symbol s, Loop l) {
        ...
        CetusAnnotation note = an.getAnnotation(CetusAnnotation.class, "private");
        if (note != null) {
